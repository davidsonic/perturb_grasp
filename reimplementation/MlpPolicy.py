...
class MlpPolicy(object):
    ...
    # def _init(self, is_pro, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
    def _init(self,  ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        # print('ob_space', ob_space.shape)
        ...
        # is this still right?
        # if is_pro:
        #     ob = U.get_placeholder(name="pro_ob", dtype=tf.float32, shape=[sequence_length] + list(ob_space.shape))
        # else:
        #     ob = U.get_placeholder(name="adv_ob", dtype=tf.float32, shape=[sequence_length] + list(ob_space.shape))
        ...
        obz = tf.clip_by_value(ob , -5.0, 5.0)
        last_out = obz
